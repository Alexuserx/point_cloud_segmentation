...
`P.S. не совсем понятно, чем обусловлено данное время инференса. Во время валидации на одно изображение (на 5 сэмплов по 4096 точек) в среднем приходится 100ms. Больше похоже на косяк, который не получилось исправить.`
...
Модель показывает высокое качество сегментации для всех трёх классов (по метрике IoU и визуально) а также инференс модели ограниченном облаке точек менее 50ms. 
